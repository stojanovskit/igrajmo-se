...
function DominoBoard({room,playerId,busy,action}:any){const hand=room.state.hands[playerId]||[],chain=room.state.chain;const left=chain[0]?.[0],right=chain.at(-1)?.[1];const fits=(tile:number[],side:'left'|'right')=>!chain.length||tile.includes(side==='left'?left:right);const playable=hand.some((tile:number[])=>fits(tile,'left')||fits(tile,'right'));return <div className="compact-game"><div className="domino-chain">{chain.length?chain.map((tile:number[],i:number)=><span key={i}>{tile[0]}│{tile[1]}</span>):<em>Постави ја првата плочка</em>}</div><div className="card-hand domino-hand">{hand.map((tile:number[],i:number)=><div key={i}><button disabled={busy||room.turn_id!==playerId||!fits(tile,'left')} onClick={()=>action('place',{index:i,side:'left'})}>←</button><span>{tile[0]}│{tile[1]}</span><button disabled={busy||room.turn_id!==playerId||!fits(tile,'right')} onClick={()=>action('place',{index:i,side:'right'})}>→</button></div>)}</div><div className="action-row"><button disabled={busy||room.turn_id!==playerId||!room.state.bag.length||playable} onClick={()=>action('draw')}>Извлечи ({room.state.bag.length})</button><button disabled={busy||room.turn_id!==playerId||!!room.state.bag.length||playable} onClick={()=>action('pass')}>Прескокни</button></div></div>}
...
function TarokBoard({room,playerId,busy,action}:any){const hand=room.state.hands[playerId]||[],lead=room.state.trick[0]?.card.slice(-1),hasLead=hand.some((card:string)=>card.endsWith(lead)),hasTarok=hand.some((card:string)=>card.endsWith('T'));const legal=(card:string)=>!lead||card.endsWith(lead)||(!hasLead&&(lead==='T'||card.endsWith('T')||!hasTarok));return <div className="compact-game"><div className="trick-area">{room.state.trick.length?room.state.trick.map((item:any,i:number)=><span key={i} className={item.card.endsWith('♥')||item.card.endsWith('♦')?'red-card':''}>{item.card}</span>):<em>Отвори рака</em>}</div><div className="card-hand">{hand.map((card:string,i:number)=><button key={i} className={card.endsWith('♥')||card.endsWith('♦')?'red-card':''} disabled={busy||room.turn_id!==playerId||!legal(card)} onClick={()=>action('play',{index:i})}>{card}</button>)}</div><p className="rules-note">Мини тарок: следи ја отворената боја; ако ја немаш, одиграј тарок. Повисокиот тарок победува.</p></div>}

function ShipsBoard({room,playerId,busy,action}:any){const opponent=room.host_id===playerId?room.guest_id:room.host_id;const myFleet=room.state.fleets[playerId]||[],theirFleet=room.state.fleets[opponent]||[],theirShots=room.state.shots[opponent]||[],myShots=room.state.shots[playerId]||[];return <div className="ships-wrap"><Sea title="Моја флота" fleet={myFleet} shots={theirShots}/><Sea title="Противничко море" fleet={theirFleet} shots={myShots} target disabled={busy||room.turn_id!==playerId} onShoot={(cell)=>action('shoot',{cell})}/></div>}
